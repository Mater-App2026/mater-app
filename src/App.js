    ...
    try {
      ...
      const res = await fetch("/api/chat", {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 500,
          system: language === "en"
            ? "You are a Catholic writer preparing a daily world prayer intention. You will be given a VERIFIED real news headline and summary from a news API. Do NOT invent additional facts, numbers, or details — base your writing strictly and only on the given headline and summary. Respond ONLY with valid JSON, in English, no code blocks: {lugar: 'country or region mentioned in the headline', descripcion: 'a respectful, factual 2-3 sentence summary of what is happening, based only on the given headline/summary', oracion: 'a 3-4 line Catholic intercessory prayer for those affected', emoji: 'one appropriate emoji'}"
            : "Eres un redactor católico que prepara una intención diaria de oración por el mundo. Se te dará un titular de noticia REAL y VERIFICADO junto con su resumen, obtenidos de una API de noticias. NO inventes datos, cifras ni detalles adicionales — basa tu redacción estricta y únicamente en el titular y resumen dados. Responde SOLO con JSON válido, en español, sin bloques de código: {lugar: 'país o región mencionado en el titular', descripcion: 'un resumen respetuoso y factual de 2-3 frases sobre lo que está pasando, basado solo en el titular/resumen dado', oracion: 'una oración católica de intercesión de 3-4 líneas por los afectados', emoji: 'un emoji apropiado'}",
          messages: [{
            role: "user",
